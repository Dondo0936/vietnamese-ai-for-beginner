...
3. Verify the deploy landed by curling a known route on `ai-edu-app.vercel.app` (e.g. `curl -sS -o /dev/null -w "%{http_code}\n" https://ai-edu-app.vercel.app/`) and expecting `200`. Grep the response body for a string you know is new in this ship to confirm the content matches HEAD, not a stale build.
