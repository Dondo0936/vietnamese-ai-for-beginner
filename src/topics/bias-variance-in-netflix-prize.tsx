...
const MODELS: ModelEntry[] = [
  {
    ...
    whyLikeThis:
      "Bias cao vì giả định mọi sở thích là cộng tuyến tính, bỏ qua tương tác phức tạp. Variance rất thấp: model fit gần như nhau dù bớt đi vài ngàn review lẻ.",
  },
  {
    id: "tree",
    name: "Decision tree",
    ...
    whyLikeThis:
      "Bias trung bình. Cây bắt được tương tác nhưng chỉ ở dạng bậc thang. Variance cũng trung bình: chỉ cần đổi cách chia dữ liệu một chút là cây đã khác.",
  },
  {
    id: "deep",
    ...
    whyLikeThis:
      "Bias thấp vì đủ linh hoạt để nắm các cụm sở thích ngách. Variance cao: chỉ cần đổi seed huấn luyện là RMSE đã chạy đi một quãng.",
  },
  {
    id: "ensemble",
    ...
    whyLikeThis:
      "Bias thấp ngang deep MF, vẫn giữ khả năng bắt mẫu phức tạp. Variance giảm mạnh nhờ hiệu ứng lỗi của mỗi mô hình lệch ngẫu nhiên sẽ bù trừ nhau khi lấy trung bình.",
  },
];
...
export default function BiasVarianceInNetflixPrize() {
  ...
  return (
    <ApplicationLayout metadata={metadata} parentTitleVi="Bias và Variance">
      ...
      >
        <p>
          Năm 2006, Netflix treo giải <strong>1 triệu đô-la</strong> cho đội nào cải thiện hệ thống
          gợi ý phim thêm 10%. Ba năm sau, hơn <strong>40 000 đội từ 186 quốc gia</strong> vẫn đang
          thử. Đội chiến thắng tên BellKor&apos;s Pragmatic Chaos. Họ nộp bài sớm hơn đội nhì đúng{" "}
          <em>22 phút</em>. Cả hai đội cùng cán mốc RMSE 0,8554 sau khi gộp hơn 800 mô hình.
        </p>
        <p>
          Vì sao để hạ 10% RMSE người ta phải gộp đến 800 mô hình? Vì sao Netflix{" "}
          <em>không triển khai</em> giải pháp thắng cuộc? Toàn bộ câu chuyện là một lớp học sống động
          về <TopicLink slug="bias-variance">Bias và Variance</TopicLink> cùng giới hạn thực tế của
          ensemble, khi chi phí tính toán cũng quan trọng ngang độ chính xác.
        </p>
      </ApplicationHero>

      <ApplicationProblem topicSlug="bias-variance-in-netflix-prize">
        <p>
          Hệ thống gợi ý ban đầu của Netflix tên là <em>Cinematch</em>. Nó tính tương quan giữa
          người dùng theo đánh giá phim. RMSE (root mean squared error, tức sai số bình phương
          trung bình) của Cinematch là <strong>0,9525</strong> trên tập kiểm tra. Netflix đặt mục
          tiêu: hạ RMSE xuống ≤ 0,8572, đúng 10% cải thiện, để nhận 1 triệu đô-la.
        </p>
        <p>
          Vấn đề cốt lõi: <strong>mỗi mô hình đơn lẻ đều mắc kẹt</strong>. Mô hình đơn giản như hồi
          quy tuyến tính có bias cao, không bắt được tương tác phức tạp giữa sở thích người xem.
          Mô hình phức tạp như deep matrix factorization có variance cao, kết quả dao động mạnh
          giữa các tập huấn luyện khác nhau. Hạ cả hai cùng lúc là điều không một mô hình đơn lẻ
          nào làm được. Nhưng ensemble thì có thể.
        </p>
      </ApplicationProblem>
      ...
      >
        <Beat step={1}>
          <p>
            <strong>SVD đơn lẻ: bias cao, cải thiện chừng 7%.</strong> Bước đầu tiên, các đội dùng
            SVD (Singular Value Decomposition, tức phân rã giá trị suy biến, một phương pháp nén ma
            trận đánh giá thành các vector ngắn gọn). SVD hạ RMSE khoảng 7%. Tuy nhiên nó vẫn giả
            định sở thích người dùng tuân theo công thức cộng tuyến tính, nên bias còn lại đáng kể.
          </p>
        </Beat>
        <Beat step={2}>
          <p>
            <strong>Bổ sung RBM và kNN: giảm bias, tăng variance.</strong> Các đội thêm RBM
            (Restricted Boltzmann Machine, tức mạng Boltzmann giới hạn, một loại mô hình xác suất
            học tương tác ẩn) và kNN (k láng giềng gần nhất, tức tìm người dùng có sở thích giống
            bạn). Mỗi mô hình mới bắt được thêm vài mẫu nhỏ, nhưng đi kèm variance cao hơn vì nhạy
        ...
        <Beat step={3}>
          <p>
            ...
            nhiễu. Đó chính là nguyên lý cốt lõi của việc giảm variance.
          </p>
        </Beat>
        <Beat step={4}>
          <p>
            <strong>Grand Prize: 800+ mô hình, RMSE 0,8554.</strong> Để cán mốc 10,06% cải thiện,
            đội chiến thắng &ldquo;BellKor&apos;s Pragmatic Chaos&rdquo; hợp nhất ba siêu đội thành
            một, dùng tổng cộng <em>hơn 800 mô hình</em>. Khoảnh khắc quyết định nằm ở thời gian
            nộp bài: họ nộp trước đội The Ensemble đúng 22 phút, dù cả hai cùng đạt RMSE giống
            nhau. Chênh lệch quy ra từng giây.
          </p>
        </Beat>
        <Beat step={5}>
          <p>
            <strong>Thực tế: Netflix chỉ triển khai 2 thuật toán.</strong> Netflix sau đó tiết lộ:
            chỉ riêng SVD và RBM đã cho 99% lợi ích. 800+ mô hình còn lại cải thiện thêm vài phần
            trăm RMSE cuối nhưng làm chi phí tính toán và độ phức tạp vận hành tăng lên{" "}
            <em>hàng chục lần</em>. Đây là bài học kinh điển:{" "}
            <strong>giảm variance bằng ensemble có thể không đáng trong sản xuất thực tế</strong>.
            Lý thuyết luôn cần đối chiếu với ràng buộc kinh doanh.
          </p>
        </Beat>
      </ApplicationMechanism>
      ...
      <ApplicationTryIt topicSlug="bias-variance-in-netflix-prize">
        ...
        <p className="text-sm text-muted leading-relaxed mb-4">
          Bấm vào một mô hình để xem biểu đồ <em>dự đoán so với thực tế</em>. Mô hình tốt là khi các
          chấm bám sát đường chéo. Quan sát cách bias (độ lệch tâm) và variance (độ rải) thay đổi
          khi bạn đi từ mô hình đơn giản đến ensemble.
        </p>
        ...
        <div className="rounded-xl border border-border bg-card p-4 mb-6">
          ...
          <p className="text-xs text-muted leading-relaxed mt-3 italic">
            Mức RMSE 0,8572 là &ldquo;vạch đích&rdquo; 10%. Chỉ riêng ensemble mới vượt qua được.
          </p>
        </div>
        ...
        <div className="mt-6">
          <InlineChallenge
            ...
            correct={1}
            explanation="Nguyên tắc cốt lõi của ensemble: nếu hai mô hình có lỗi độc lập, variance của trung bình sẽ chỉ bằng một nửa variance của từng mô hình. Trong thực tế lỗi không hoàn toàn độc lập, nhưng vẫn có tương quan âm đủ để trung bình tốt hơn. Đó là lý do Netflix Prize cần tới 800+ mô hình. Càng nhiều góc nhìn khác nhau, variance càng triệt tiêu hiệu quả."
          />
        </div>
        ...
        <div className="mt-4">
          <InlineChallenge
            ...
            options={[
              ...
              "Vì chi phí tính toán và vận hành 800+ mô hình vượt xa lợi ích cải thiện vài phần trăm RMSE cuối. Bài học kinh điển về 'đủ tốt' so với 'tối ưu'",
              "Vì đội thắng không chia sẻ code",
            ]}
            correct={2}
            explanation="RMSE 0,8554 đẹp trên bảng xếp hạng, nhưng chạy 800 mô hình cho 100 triệu người dùng thì tốn GPU và CPU khủng khiếp, dễ lỗi và cực khó bảo trì. 99% lợi ích đã đến từ SVD và RBM. Đây là nguyên tắc sản xuất kinh điển: tối ưu 'đủ dùng' thường thắng tối ưu 'tuyệt đối'. Cùng tư tưởng này có trong bài toán linear so với deep learning. Khi baseline đạt 85%, deep chỉ thêm 0 đến 15% và thường không đáng."
          />
        </div>
            ...
            {[
              <div key="e1" className="rounded-lg bg-surface/60 border border-border p-4">
                <p className="text-sm text-foreground leading-relaxed">
                  ...
                  sao. Thực tế người dùng chấm 4 sao. Cả ba đều sai, nhưng mỗi mô hình sai theo
                  một kiểu khác nhau.
                </p>
              </div>,
              <div key="e2" className="rounded-lg bg-surface/60 border border-border p-4">
                <p className="text-sm text-foreground leading-relaxed">
                  Chìa khoá là các sai lệch <em>không theo cùng một hướng</em>. SVD bỏ qua thể loại
              ...
              <div key="e3" className="rounded-lg bg-surface/60 border border-border p-4">
                <p className="text-sm text-foreground leading-relaxed">
                  Trung bình ba dự đoán: (3,7 + 4,2 + 3,5) / 3 = 3,8 sao. Gần với giá trị thật 4
                  sao hơn mọi mô hình riêng lẻ. Toán học đứng sau điều này: nếu sai số độc lập và
                  có trung bình gần 0, variance của <em>trung bình n mô hình</em> chỉ bằng{" "}
                  <em>1/n</em> variance của một mô hình.
                </p>
              </div>,
              <div key="e4" className="rounded-lg bg-surface/60 border border-border p-4">
                <p className="text-sm text-foreground leading-relaxed">
                  Tuy nhiên vẫn có một <strong>giới hạn</strong>. Nếu cả ba mô hình đều lệch cùng
                  hướng (ví dụ tất cả đều dự đoán thấp hơn sự thật 0,3 sao), trung bình vẫn lệch
                  0,3 sao. Ensemble giảm <em>variance</em>, không giảm <em>bias</em>. Vì vậy đội
                  Netflix kết hợp những mô hình <em>khác nhau về bản chất</em>, để bias của chúng
                  cũng phân tán theo nhiều hướng.
                </p>
              </div>,
            ]}
          </StepReveal>
        ...
        <div className="mt-8">
          ...
          <p className="text-sm text-muted leading-relaxed mb-4">
            Câu chuyện Netflix Prize còn kịch tính hơn cả mô hình, vì nó kéo dài 3 năm với nhiều
            khúc ngoặt. Dưới đây là bốn cột mốc quan trọng nhất.
          </p>
          <div className="space-y-3">
            ...
            <TimelineEvent
              ...
              body="Đội AT&T Labs đi đầu với kỹ thuật SVD cải tiến và RBM. Nhưng còn cách mốc 10% khá xa, cần thêm các mô hình mới."
              color="#10b981"
            />
            <TimelineEvent
              date="06/2009"
              title="Ba đội hàng đầu hợp nhất"
              body="Ba đội hàng đầu là BellKor, BigChaos, Pragmatic Theory hợp nhất thành BellKor's Pragmatic Chaos để tận dụng lẫn nhau. Quyết định này rút ngắn đường đến mốc 10%."
              color="#a855f7"
            />
            <TimelineEvent
              date="26/07/2009 18:42 UTC"
              title="Hai đội cán đích cùng lúc"
              body="BellKor's Pragmatic Chaos nộp bài với RMSE 0,8554. Đúng 20 phút sau, đội The Ensemble nộp kết quả giống hệt. BellKor thắng vì nộp trước. Giải thưởng được trao tháng 9/2009."
              color="#f59e0b"
            />
          </div>
        </div>
        ...
        <div className="mt-8">
          ...
          <p className="text-sm text-muted leading-relaxed mb-4">
            Khi đặt lên bàn cân giữa SVD cộng RBM và ensemble 800+, Netflix nhìn vào ba trục, không
            chỉ riêng RMSE. Đây là dạng đánh giá mà mọi đội ML trưởng thành đều phải làm trước khi
          ...
          <Callout variant="info" title="Netflix chọn gì cuối cùng?">
            Kết quả: Netflix triển khai phiên bản gọn với SVD và RBM, đạt khoảng 99% lợi ích so với
      ...
      >
        <Metric
          value="RMSE ban đầu (Cinematch) là 0,9525. RMSE thắng cuộc 0,8554, đúng 10,06% cải thiện"
          sourceRef={1}
        />
        <Metric
        ...
        />
        <Metric
          value="Đội thắng nộp bài sớm hơn đội nhì chỉ 22 phút, cả hai đạt cùng một RMSE"
          sourceRef={2}
        />
        <Metric
          value="Netflix chỉ triển khai SVD và RBM, hai thuật toán đủ cho 99% lợi ích"
          sourceRef={3}
        />
      </ApplicationMetrics>
      ...
      >
        <p>
          Không hiểu đánh đổi bias và variance, các đội thi sẽ mắc kẹt ở hai đầu. Một là dùng mô
          hình đơn giản rồi chấp nhận RMSE 0,95 (bias cao). Hai là xây mô hình cực phức tạp rồi
          thất vọng khi nó chạy kém trên dữ liệu mới (variance cao). Không ai vượt qua được
          ngưỡng 10%.
        </p>
        <p>
          Hiểu đánh đổi bias và variance mở ra lối thoát: kết hợp nhiều mô hình khác nhau
          (ensemble) để giảm variance mà không tăng bias. Tuy nhiên bài học Netflix cũng cho thấy
          mặt trái. Ensemble quá lớn có thể &ldquo;thắng cuộc thi nhưng thua sản xuất&rdquo;, một
          lời nhắc rằng lý thuyết luôn cần đối chiếu với ràng buộc vận hành. Đây là lý do các công
          ty công nghệ lớn luôn dành một khe thời gian cho câu hỏi:{" "}
          <em>tối ưu thêm 1% RMSE có xứng với chi phí chạy thêm 100 GPU không?</em>
        </p>
      </ApplicationCounterfactual>
