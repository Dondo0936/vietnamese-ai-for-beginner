    ...
    <>
      ...
      <LessonSection step={1} totalSteps={TOTAL_STEPS} label="Dự đoán">
        <PredictionGate
          ...
          correct={1}
          explanation="SLM 8B xử lý FAQ đơn giản rất tốt (accuracy 90%+). Tự host trên 1 GPU tốn khoảng $3/ngày, so với $150/ngày khi gọi API, giảm 50 lần chi phí. Với nhu cầu hàng ngày, bạn không cần đến model lớn nhất."
        >
          ...
          <div className="space-y-4">
            ...
            <svg viewBox="0 0 600 180" className="w-full max-w-2xl mx-auto">
              ...
              {MODELS.map((m, i) => {
                ...
                return (
                  <g key={i}>
                    <text x={15} y={y + 14} fill={isActive ? "#e2e8f0" : "#64748b"} fontSize={11} fontWeight={isActive ? "bold" : "normal"}>
                      {m.name}
                    </text>
      ...
      <LessonSection step={3} totalSteps={TOTAL_STEPS} label="Khoảnh khắc Aha">
        <AhaMoment>
          <p>
            SLM không làm được mọi thứ như GPT-4,
            nhưng làm tốt những việc bạn cần hàng ngày. Phi-3 Mini (3.8B) đạt <strong>MMLU 69%</strong>,{" "}ngang GPT-3.5. Bí quyết không phải nhiều tham số, mà là{" "}
      ...
      <LessonSection step={4} totalSteps={TOTAL_STEPS} label="Thử thách">
        <InlineChallenge
          ...
          correct={1}
          explanation="HIPAA cấm gửi data lên cloud → chỉ có on-device, iPhone 15 có 8GB RAM (chia với OS, còn ~4GB cho app). Phi-3 Mini INT4: ~2GB RAM, chạy được. Llama 70B: 35GB RAM, không thể. Privacy, performance và size đều đạt, nên SLM là lựa chọn phù hợp ở đây."
        />
      </LessonSection>
