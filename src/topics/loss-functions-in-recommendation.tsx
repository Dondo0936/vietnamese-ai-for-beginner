...
/* ────────────────────────────────────────────────────────────
   ...
   "Minh, 20 tuổi, Sài Gòn, thích bóng rổ và lo-fi"
   ──────────────────────────────────────────────────────────── */
...
> = {
  mse: {
    labelVi: "MSE: dự đoán đúng con số",
    labelShort: "MSE (pointwise)",
    color: "#0ea5e9",
    tagline:
      "Bắt model đoán đúng con số rating hay watch-time. Nhưng việc sắp xếp lại không phải mục tiêu của nó.",
    icon: Target,
    scoreField: "mseScore",
  },
  pairwise: {
    ...
    color: "#10b981",
    tagline:
      "Chỉ cần model đoán đúng A đứng trên B, khớp với hành vi thật. Số cụ thể không quan trọng.",
    icon: GitCompare,
  ...
  listwise: {
    ...
    color: "#f59e0b",
    tagline:
      "Nhìn toàn bộ top-K, phạt nặng lỗi ở vị trí trên. Đây là thứ mỗi app thực sự đang tối ưu.",
    icon: ListOrdered,
...
export default function LossFunctionsInRecommendation() {
  ...
  return (
    <ApplicationLayout metadata={metadata} parentTitleVi="Hàm loss">
      {/* ═════════════ HERO ═════════════ */}
      <ApplicationHero
        parentTitleVi="Hàm loss"
        topicSlug="loss-functions-in-recommendation"
      >
        <p>
          Mở <strong>Shopee</strong> và bạn thấy &ldquo;Gợi ý cho bạn&rdquo;
          với 20 sản phẩm xếp theo thứ tự. Mở <strong>TikTok</strong>, lướt
          For You, và 30 video đầu tiên đã được chọn thứ tự rất kỹ. Mở{" "}
          <strong>Spotify</strong>, Discover Weekly sắp 30 bài theo đúng
          tuần tự bạn sẽ thích nhất rồi giảm dần. <strong>YouTube</strong>{" "}
          làm điều tương tự với hàng tỉ video mỗi ngày.
        </p>
        <p>
          Bên trong cả bốn hệ gợi ý này là cùng một câu hỏi: <em>model
          nên học điều gì?</em> Câu trả lời nằm ở{" "}
          <strong>hàm loss</strong> (loss function), thước đo cho biết
          model đoán đúng đến đâu. Cùng một dữ liệu, đổi loss thì bảng
          xếp hạng top 10 đổi hoàn toàn. Chỗ thú vị nhất của câu chuyện
          không phải &ldquo;bao nhiêu dữ liệu&rdquo; mà là &ldquo;dạy
          model tối ưu điều gì&rdquo;. Đó mới là cái quyết định trải
          nghiệm của bạn.
        </p>
        ...
        <div className="not-prose grid grid-cols-2 gap-2 sm:grid-cols-4 my-4">
          {[
            ...
            { name: "TikTok", tag: "Video (CN)", color: "#ec4899", icon: Video, note: "For You sắp video cho người xem lâu, ít bỏ lướt." },
            { name: "Spotify", tag: "Nhạc (SE)", color: "#22c55e", icon: Music, note: "Discover Weekly: 30 bài theo đúng thứ tự yêu thích." },
            { name: "YouTube", tag: "Video (US)", color: "#ef4444", icon: PlayCircle, note: "Home và Up Next tối ưu watch time, chống clickbait." },
          ].map((p) => {
            const Icon = p.icon;
            return (
              <div
        ...
        <Callout variant="insight" title="Câu hỏi trung tâm của bài này">
          Khi bạn click video A và lướt bỏ video B, hàm loss nên{" "}
          <em>thưởng</em> cho model điều gì? Thưởng vì đoán đúng con số
      ...
      <ApplicationProblem topicSlug="loss-functions-in-recommendation">
        <p>
          ...
          <strong> sắp xếp đúng thứ tự</strong>. Nhưng các hàm loss quen
          thuộc nhất từ trường lớp (MSE là mean squared error, MAE là
          mean absolute error) lại được thiết kế cho regression. Chúng
          phạt theo khoảng cách giữa con số dự đoán và con số thật.
        </p>

        <div className="not-prose grid gap-3 sm:grid-cols-3 my-4">
          {[
            { icon: Target, title: "MSE: học con số", desc: "Dự đoán rating 4.0 khi thật là 4.5 thì mất 0.25. Phạt theo khoảng cách, không quan tâm thứ tự.", color: "#0ea5e9", when: "Regression giá nhà, nhiệt độ. Có đáp án số thật." },
            { icon: GitCompare, title: "Pairwise: học cặp", desc: "So từng cặp A và B. Model đặt A > B đúng thì được điểm tốt. Không cần biết số cụ thể.", color: "#10b981", when: "Hệ gợi ý có phản hồi ngầm: ai click, ai skip." },
            { icon: ListOrdered, title: "Listwise: học cả bảng", desc: "Nhìn top-K, phạt mạnh lỗi ở vị trí cao. Tối ưu trực tiếp metric đánh giá (NDCG).", color: "#f59e0b", when: "Ranking sản xuất ở TikTok, YouTube, Shopee." },
          ].map((c) => {
            const Icon = c.icon;
            return (
              <div
        ...
        <p>
          ...
          rate) bằng binary classification loss. Kết quả là trang chủ
          tràn ngập clickbait, tức là video tiêu đề giật gân nhưng nội
          dung kém. Họ chuyển sang <strong>weighted logistic regression
          với trọng số watch-time</strong>. Về bản chất, loss biến thành
          &ldquo;dự đoán thời gian xem&rdquo; thay vì &ldquo;có click
          hay không&rdquo;. Trải nghiệm thay đổi chỉ sau vài tuần chạy
          thử.
        </p>

        <Callout variant="warning" title="Bẫy lớn nhất: dùng nhầm loss">
          Dùng MSE cho bài toán ranking là ép model học con số rating
          tuyệt đối. Nhưng người dùng không cảm được khoảng cách giữa
          rating 7.8 và 8.3. Họ chỉ biết &ldquo;video này hay hơn video
          kia&rdquo;. Pairwise loss học chính xác cái thứ tự đó.
        </Callout>
      </ApplicationProblem>
      ...
      >
        <Beat step={1}>
          <p>
            <strong>MSE và RMSE: kỷ nguyên Netflix Prize (2006-2009).</strong>{" "}
            Netflix mở cuộc thi 1 triệu đô với metric là RMSE trên rating
            1-5 sao. Toàn ngành học bằng MSE trong 3 năm. Bài báo thắng
            giải dùng tới 107 model gộp lại. Nhưng khi triển khai thật,
            Netflix lại không dùng. Lý do: metric được tối ưu (RMSE trên
            sao) không phải thứ người xem quan tâm. Khách hàng không
            nhập sao, họ ấn play.
          </p>
        </Beat>
        <Beat step={2}>
          <p>
            <strong>Pairwise loss (BPR là Bayesian Personalized Ranking,
            2009).</strong> Rendle và cộng sự đề xuất: thay vì dự đoán
            số, hãy cho model học từ cặp &ldquo;A được click, B không
            được click&rdquo;. Hàm loss phạt khi model chấm A ≤ B. Không
            cần rating rõ ràng, chỉ cần biết người dùng thích A hơn B. Đây
            là nền tảng của gần như mọi hệ gợi ý hiện đại có phản hồi
            ngầm.
          </p>
        </Beat>
        <Beat step={3}>
          <p>
            <strong>Listwise và LambdaRank (Microsoft, 2010).</strong>{" "}
            Burges et al. chỉ ra: pairwise đối xử mọi cặp như nhau, nhưng
            lỗi ở top 1-3 tệ hơn nhiều so với lỗi ở vị trí 18-20.
            LambdaRank đưa thêm <em>trọng số NDCG</em> vào gradient,
            phạt mạnh hơn khi lỗi xảy ra ở đỉnh bảng. Đây chính là thuật
            toán phía sau Bing search rank và lan ra hầu hết hệ gợi ý
            lớn.
          </p>
        </Beat>
        <Beat step={4}>
          <p>
            ...
            ra cách mượn logistic regression để tối ưu thời gian xem.
            Mỗi ví dụ positive được nhân trọng số bằng số giây đã xem.
            Loss function vẫn là cross-entropy, nhưng trọng số biến nó
            thành &ldquo;xác suất người dùng sẽ xem bao lâu&rdquo;. Kết
            quả: video clickbait bị hạ rank vì ai nhấp cũng thoát nhanh,
            trọng số thấp.
          </p>
        </Beat>
        <Beat step={5}>
          <p>
            ...
            Model có nhiều đầu ra (đã xem hết, like, comment, share) và
            cộng nhiều loss lại với trọng số riêng. Skip nhanh là tín
            hiệu negative mạnh, like là positive mạnh, share là cực
            mạnh. Tối ưu tất cả cùng lúc, ranking phản ánh nhiều dạng
            hài lòng hơn.
          </p>
        </Beat>
        <Beat step={6}>
          <p>
            <strong>Shopee và Spotify: cùng một công thức.</strong>{" "}
            Shopee tối ưu xác suất mua (chứ không phải click) với loss
            có trọng số doanh thu. Spotify Discover Weekly kết hợp
      ...
      <section className="mb-12 rounded-2xl border-2 border-accent/30 bg-accent-light/30 p-5 space-y-5">
        ...
        <p className="text-sm text-muted leading-relaxed">
          Dưới đây là <strong>Minh</strong>, 20 tuổi, ở Sài Gòn. Lịch sử
          xem gồm bóng rổ NBA, lo-fi ôn bài, podcast tuổi trẻ. Model đã
        ...
        <Callout variant="insight" title="Quan sát chìa khoá">
          Thử ba loss. Với <strong>MSE</strong>, clickbait lọt top 3 và
          NDCG@10 thấp nhất. Với <strong>pairwise</strong>, clickbait bị
          đẩy xuống cuối bảng. Với <strong>listwise</strong>, top 3 toàn
          video bóng rổ và lo-fi đúng gu Minh. Cùng một model, khác loss
          thì khác trải nghiệm.
        </Callout>
      </section>
      ...
      <section className="mb-12 space-y-5">
        ...
        <p className="text-sm text-muted leading-relaxed">
          Ba loss xử lý <em>cùng một cặp dữ liệu</em> theo cách rất
          khác nhau. Khi Minh click video A và lướt bỏ video B, mỗi
          loss tạo ra một tín hiệu học riêng. Bấm qua lại để thấy cùng
          một cặp dưới ba góc nhìn.
        </p>

        <ToggleCompare
          labelA="Pointwise (MSE)"
          labelB="Pairwise (BPR)"
          description="Cùng dữ liệu: Minh click video A (bóng rổ) và lướt bỏ video B (clickbait). Hai loss xử lý ra sao?"
      ...
      <section className="mb-12 space-y-5">
        ...
        <p className="text-sm text-muted leading-relaxed">
          Bốn bước dưới đây mô tả cùng một lần cập nhật model. Mỗi bước
          cho thấy loss đang yêu cầu điều gì.
        </p>
      ...
      >
        <Metric
          value="YouTube chuyển từ CTR loss sang weighted cross-entropy với trọng số watch-time, loại bỏ hiệu quả clickbait khỏi top gợi ý"
          sourceRef={1}
        />
        <Metric
        ...
        />
        <Metric
          value="BPR pairwise loss (Rendle 2009) trở thành chuẩn mực cho hệ gợi ý có phản hồi ngầm. Được trích dẫn hơn 10.000 lần."
          sourceRef={3}
        />
        <Metric
        ...
        />
        <Metric
          value="Neural collaborative filtering (He 2017) cho thấy thay pointwise bằng pairwise loss cải thiện HR@10 và NDCG@10 từ 3 đến 8% trên dataset MovieLens và Pinterest"
          sourceRef={6}
        />
        <Metric
      ...
      <ApplicationTryIt topicSlug="loss-functions-in-recommendation">
        ...
        <div className="space-y-4">
          <InlineChallenge
            question="Minh click video A (bóng rổ) và lướt bỏ video B (clickbait) chỉ sau 2 giây. Hàm loss nên thưởng model điều gì?"
            options={[
              "Thưởng vì đoán đúng con số rating của cả A và B (học điểm chính xác)",
              "Thưởng vì đặt A cao hơn B trong danh sách, kể cả khi không biết rating chính xác",
              "Thưởng vì dự đoán nhiều click cho cả hai video",
              "Không cần thưởng gì, lướt 2 giây là tín hiệu yếu, bỏ qua",
            ]}
            correct={1}
            explanation="Chính là ý tưởng của pairwise loss (BPR). Minh không gắn sao cho A hay B, chỉ có tín hiệu ngầm 'A được click, B bị lướt'. Pairwise loss chỉ cần model đặt A > B. Watch-time ngắn của B còn là tín hiệu negative rõ ràng, giúp phân biệt clickbait với nội dung thật sự hấp dẫn. Đây là lý do pairwise thay thế MSE ở gần như mọi hệ gợi ý hiện đại."
          />

          <InlineChallenge
            question="Bạn là kỹ sư Shopee, cần chọn loss cho model xếp hạng sản phẩm ở 'Gợi ý cho bạn'. Shopee kiếm tiền khi người dùng MUA chứ không phải chỉ click. Loss nào phù hợp nhất?"
            options={[
              "MSE thuần túy trên số lượng click",
              "Binary cross-entropy chỉ trên tín hiệu click",
              "Weighted loss với mỗi ví dụ positive nhân trọng số bằng giá trị đơn hàng, hoặc multi-task loss kết hợp click với mua",
              "Không cần loss, dùng quy tắc 'sản phẩm rẻ nhất lên đầu'",
            ]}
            correct={2}
            explanation="Đây là bài học trực tiếp từ YouTube (watch-time) áp dụng cho e-commerce. Click không phải mục tiêu cuối, mua mới là. Nếu loss chỉ tối ưu click, sản phẩm clickbait (ảnh hấp dẫn, nội dung kém) sẽ lên top. Trọng số theo giá trị đơn hàng (hoặc multi-task với click cộng add-to-cart cộng purchase) làm loss phản ánh đúng thứ Shopee muốn tối ưu. TikTok Monolith dùng công thức tương tự với nhiều tín hiệu song song."
          />

          <InlineChallenge
            question="Trong danh sách 10 gợi ý, model A đặt video hay nhất ở vị trí #1 nhưng có video dở ở #5. Model B đặt video hay nhất ở #3 nhưng toàn bộ #1 đến #5 đều ổn. Loss nào sẽ chấm model A cao hơn model B?"
            options={[
          ...
          <InlineChallenge
            ...
            options={[
              "MSE trên số giây nghe, ép model đoán chính xác thời lượng",
              "BPR pairwise với positive là played-through, negative là skipped, có thể cộng thêm weighted loss theo thời gian nghe",
              "Dùng accuracy (tỉ lệ đoán đúng label nhị phân) làm loss trực tiếp",
              "Không cần loss vì Spotify đã biết bài nào hay",
            ]}
            correct={1}
            explanation="Spotify và TikTok đều dùng cách tiếp cận này: tín hiệu ngầm (skip với play-through) làm nền cho pairwise. Cộng thêm trọng số watch-time (hoặc listen-time) thì loss trực tiếp tối ưu 'người dùng nghe bao lâu'. MSE trên số giây cụ thể gặp vấn đề vì user không cảm được khoảng cách giữa 45s và 48s. Accuracy không được dùng làm loss vì nó không có gradient mượt, model không học được từ nó."
          />
        </div>

        <div className="mt-6 space-y-4">
          <Callout variant="tip" title="Quy tắc thực chiến: hỏi ba câu trước khi chọn loss">
      ...
      >
        ...
        <p>
          ...
          đổi kiến trúc mạng, không cần GPU mạnh hơn. Nhưng nó thay đổi
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-foreground leading-relaxed">
        ...
        không phải thứ Minh cảm thấy. Minh chỉ click A và lướt B. Kết
        quả: MSE phạt nặng vì mô hình đoán lệch số, dù thứ tự có thể
        đúng hoặc sai.
      </p>
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-foreground leading-relaxed">
        ...
        chỉ cần đúng thứ tự. Loss là hàm sigmoid của chênh lệch điểm
        giữa A và B.
      </p>
      <div className="rounded-xl border border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/20 p-4 space-y-3">
        ...
        <p className="inline-flex items-center justify-center gap-1 w-full text-xs text-foreground leading-relaxed text-center">
...
function ListwiseTopView() {
  return (
    <WeightChart
      ...
      outro="Trọng số giảm mạnh theo vị trí. Mô hình ưu tiên xếp đúng top 3 thay vì vị trí 15-20. Đây chính là thứ YouTube, TikTok, Shopee đang tối ưu thực tế."
  ...
  return (
    <div className="rounded-xl border border-border bg-surface/60 p-5 space-y-3">
      <p className="text-sm text-foreground leading-relaxed">
        ...
        &ldquo;phản hồi ngầm&rdquo; (implicit feedback). Không ai gắn sao, không có rating
        rõ ràng.
      </p>
    ...
    <StepNote
      ...
      note={
        <>
          ...
          cao&rdquo;. Trên thực tế, hầu hết người click đều thoát ngay.
        </>
      }
    ...
    <StepNote
      body={
        <>
          ...
          trọng số NDCG, tức là thay đổi nào làm NDCG tăng nhiều nhất sẽ nhận gradient lớn
          nhất. Đây là thứ đang chạy thật ở YouTube ranking, TikTok For You, Shopee search.
        </>
      }
      noteIcon={TrendingUp}
      note={
