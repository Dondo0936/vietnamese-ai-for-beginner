...
const STRATEGIES: Strategy[] = [
  {
    id: "bear",
    title: "Gấu chậm: early submission với một single model mạnh",
    animal: "gau",
    color: "#92400e",
  ...
  {
    id: "fox",
    title: "Cáo nhanh: bám public leaderboard từng ngày",
    animal: "cao",
    color: "#b45309",
  ...
  {
    id: "owl",
    title: "Cú khôn: tin CV, ensemble lớn, final submission đa dạng",
    animal: "cu",
    color: "#78350f",
      ...
      {revealed && (
        <div className="rounded-xl border border-accent/30 bg-accent-light p-3">
          <p className="text-sm text-foreground/90 leading-relaxed">
            ...
            nội bộ, tụt ở public nhưng leo ở private. Shakeup không phải may
            rủi. Nó là hình phạt cho những ai overfit vào phần công khai của
            test.
          </p>
        </div>
      )}
  ...
  const LAYERS = [
    {
      ...
      note: "Mỗi model học một pattern hơi khác nhau. Đa dạng hoá kiến trúc quan trọng hơn chọn 'model tốt nhất', vì ensemble cần lỗi độc lập để triệt nhiễu.",
    },
    {
      title: "Tầng 2: meta models, ba bộ kết hợp",
      models: [
        { name: "XGBoost meta", color: "#a855f7" },
        { name: "Neural meta", color: "#a855f7" },
        { name: "AdaBoost meta", color: "#a855f7" },
      ],
      note: "Nhận đầu ra của 33 model tầng 1 làm feature, học cách gán trọng số tối ưu. Out-of-fold predictions là bắt buộc để tránh leak.",
    },
    {
...
function FinalSubmissionPicker() {
  ...
  const SUBS: {
  ...
  }[] = [
    {
      ...
      diversity: "low",
      story:
        "35 model tầng 1 cộng meta XGBoost. CV và public khớp nhau, một dấu hiệu tốt. Chọn làm bài nộp 'an toàn'.",
    },
    {
      id: "single-strong",
      ...
      diversity: "low",
      story:
        "Model đơn lẻ nhưng được tune kỹ với Optuna. Có thể mạnh nếu CV chiến lược tốt. Dùng làm backup nếu ensemble bị shake.",
    },
    {
      id: "risky-ensemble",
      label: "Ensemble nhỏ tập trung edge case",
      cv: 0.836,
      publicLB: 0.828,
      privateExpected: "Top 5-500 (biến động lớn)",
      diversity: "high",
      story:
        "8 model đặc biệt tập trung vào các edge case. CV hơi thấp nhưng DIVERSE so với ensemble lớn. Nếu private có nhiều edge case, bài này bay lên top. Nếu không, tụt xa.",
    },
    {
      id: "copy-public",
      ...
      diversity: "low",
      story:
        "Public cao nhưng CV thấp, tức là đã overfit public test subset. KHÔNG chọn vì shakeup sẽ đánh rớt hàng trăm bậc.",
    },
  ];
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 space-y-3">
      ...
      <p className="text-xs text-muted leading-relaxed">
        Kaggle cho mỗi đội nộp tối đa hai bài final. Nguyên tắc của
...
function CvStrategyDiagram() {
  const VARIANTS = [
    ...
    },
    {
      ...
      when: "Dữ liệu theo thời gian. Huấn luyện trên quá khứ, validate trên tương lai",
      color: "#f59e0b",
    },
  ];
  return (
...
export default function ModelEvaluationSelectionInKaggle() {
  return (
    ...
    >
      ...
      >
        <p>
          Kaggle là nền tảng thi đấu khoa học dữ liệu của Google, nơi hàng chục
          nghìn kỹ sư và nhà nghiên cứu cạnh tranh trên cùng bộ dữ liệu. Giải
          thưởng lên tới hàng triệu đô la. Thứ hạng được quyết định bởi từng
          phần nghìn của metric đánh giá.
        </p>
        <p>
          Bài học lớn nhất từ Kaggle:{" "}
          <strong>bạn không bao giờ thắng bằng đúng một chiến thuật cố định</strong>.
          Có <em>gấu chậm</em> (nộp sớm một model mạnh), có <em>cáo nhanh</em>{" "}
          (tune theo public leaderboard mỗi ngày), có <em>cú khôn</em> (tin CV
          nội bộ, ensemble lớn, final submission đa dạng). Ba tính cách dẫn tới
          ba kết thúc khác nhau khi private leaderboard được công bố.
        </p>
      </ApplicationHero>
      ...
      <ApplicationProblem topicSlug="model-evaluation-selection-in-kaggle">
        <p>
          ...
          (75% còn lại, chỉ công bố khi cuộc thi kết thúc). Mục đích là tránh
          việc các đội tune tham số chỉ theo public.
        </p>
        <p>
          Hiện tượng <em>leaderboard shakeup</em> xảy ra lúc private được lật
          lên. Nhiều đội đứng top 10 public rớt hàng trăm bậc vì đã overfit
          vào subset công khai mà không biết. Đội khác, chậm ở public, lại leo
          vài chục bậc. Làm thế nào để biết một model thực sự tốt hay chỉ may
          mắn? Làm thế nào để chọn đúng hai bài nộp cuối từ hàng trăm thí
          nghiệm? Đây là câu chuyện của việc đánh giá và chọn model ở độ khó
          cao nhất.
        </p>
      </ApplicationProblem>
      ...
      >
        <Beat step={1}>
          <p>
            ...
            quan tốt, hãy tin CV chứ không phải public.
          </p>
        </Beat>
        <Beat step={2}>
          <p>
            ...
            khi viết dòng code đầu tiên, vì metric quyết định loss function
            nào để huấn luyện, threshold nào để cut-off, và cách tối ưu
            ensemble. Ví dụ: log-loss phạt nặng dự đoán tự tin sai, nên phải
        ...
        <Beat step={3}>
          <p>
            ...
            thắng Kaggle tháng 4/2025 dùng ba tầng: tầng 1 gồm 33 model, tầng
            2 có ba meta (XGBoost, neural net, AdaBoost), tầng 3 là trung bình
        ...
        <Beat step={4}>
          <p>
            <strong>
              Final submission strategy: đa dạng là tất cả.
            </strong>{" "}
            Kaggle cho phép hai bài nộp cuối. Grandmaster áp dụng nguyên tắc{" "}
            ...
            thắng cùng hoặc thua cùng, tức là mất lợi thế đa dạng. Quyết định
            này thường phân biệt top 10 với top 100.
          </p>
        </Beat>
      </ApplicationMechanism>
      ...
      <ApplicationTryIt topicSlug="model-evaluation-selection-in-kaggle">
        ...
        <p className="text-sm text-muted leading-relaxed mb-4">
          ...
          leaderboard suốt tám tuần. Khi cuộc thi kết thúc, nhấn <em>Công bố
        ...
        <div className="mt-6">
          <InlineChallenge
            ...
            options={[
              ...
              "Cáo nhanh đã tune tham số theo feedback public LB, vô tình overfit vào 25% test công khai và không tổng quát cho 75% private",
              "Kaggle sai số ngẫu nhiên",
              "Ensemble nhỏ không đủ mạnh",
            ]}
            correct={1}
            explanation="Đây là shakeup kinh điển. Public LB chỉ là một subset. Nếu bạn chọn model dựa trên điểm public hằng ngày, bạn đang fit một tập test rất bé. 75% private có phân phối khác chút, và model đã overfit sẽ sập. Tin CV hơn public LB luôn là nguyên tắc sống còn."
          />
        </div>
        ...
        <p className="text-sm text-muted leading-relaxed mb-4">
          Grandmaster không chỉ huấn luyện nhiều model. Họ <em>kiến trúc</em>{" "}
          ...
          <Callout variant="insight" title="Ba nguyên tắc stacking">
            <ol className="list-decimal pl-5 space-y-1 mt-1">
              ...
              <li>
                ...
                Dùng prediction train sẽ leak. Meta model học thuộc, và
                private sẽ sập.
              </li>
              <li>
        ...
        <p className="text-sm text-muted leading-relaxed mb-4">
          ...
          Thử nhấn hai thẻ. Hệ thống sẽ chấm chiến thuật của bạn.
        </p>
        ...
        <div className="mt-6">
          <InlineChallenge
            ...
            options={[
              "Copy ngay vì public LB cao hơn là tốt hơn",
            ...
            correct={1}
            explanation="Public LB cao mà CV thấp là dấu hiệu overfit, và shakeup sẽ lật kết quả. Grandmaster luôn chạy CV trên mọi model, kể cả model public. Chỉ tin public LB khi nó tương quan với CV nội bộ. Nếu tin được, có thể blend để tăng diversity."
          />
        </div>
        ...
        <p className="text-sm text-muted leading-relaxed mb-4">
          ...
          &ldquo;trộn nhiều model&rdquo;. Khi hiểu hết, bạn đã nắm 90% vì sao
          ...
          {[
            ...
            >
              <p className="text-sm text-foreground leading-relaxed">
                ...
                model đều không hoàn hảo, và mỗi model có kiểu sai riêng.
              </p>
            </div>,
            <div
              key="k2"
            ...
            >
              <p className="text-sm text-foreground leading-relaxed">
                ...
                và đây chính là <em>tính độc lập</em> của lỗi mà ensemble cần.
              </p>
            </div>,
            <div
              key="k3"
            ...
            >
              <p className="text-sm text-foreground leading-relaxed">
                ...
                amount lớn, LightGBM đúng hơn khi category hiếm, và meta model
                phát hiện pattern này rồi gán trọng số động.
              </p>
            </div>,
          ]}
        </StepReveal>
        ...
        <p className="text-sm text-muted leading-relaxed mb-4">
          ...
          <code>oof_preds</code>, tức là prediction trên fold mà model chưa thấy.
        </p>
...
print("OK nếu < 0.90. Nếu > 0.95, hai bài gần như giống nhau, chọn lại.")`}
        ...
        <Callout
          variant="warning"
          title="Cạm bẫy lớn nhất là nhìn public LB quá nhiều"
        >
      ...
      >
        <p>
          Không có chiến thuật đánh giá, bạn huấn luyện một model, thấy public
          LB cao, tự tin nộp bài, rồi rớt hàng trăm bậc khi private công bố.
