  ...
  return (
    <>
      ...
      <LessonSection step={2} totalSteps={TOTAL_STEPS} label="Khoanh khac Aha">
        <AhaMoment><p>Gradient descent giong <strong>di xuong nui trong suong mu</strong>: khong nhin thay dinh nhung <strong>cam nhan doc</strong>{" "}(gradient) roi buoc nguoc lai. Chain rule la <strong>chuyen phat nhanh</strong>: moi layer chi can tinh local gradient, <strong>chuyen nguoc</strong>{" "}cho layer truoc. 100 layers × local = efficient. Day la toan dang sau <strong>moi neural network</strong>!</p></AhaMoment>
      </LessonSection>

      <LessonSection step={3} totalSteps={TOTAL_STEPS} label="Thu thach">
        <InlineChallenge question="f(x) = (3x + 2)^2. Tinh f'(x) tai x=1 bang chain rule." options={["f'(1) = 50", "f'(x) = 2(3x+2)*3 = 6(3x+2). f'(1) = 6*(3+2) = 30", "f'(1) = 25"]} correct={1} explanation="Chain rule: f = u^2, u = 3x+2. df/dx = df/du * du/dx = 2u * 3 = 6(3x+2). Tai x=1: f'(1) = 6*5 = 30. Trong neural network: u = linear layer, f = activation. Backprop tinh df/dx bang cach nhan local gradients: df/du (activation grad) * du/dx (linear grad)." />
      </LessonSection>
